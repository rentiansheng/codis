...
codis-deps:
	...
	@make --no-print-directory -C ${GOPATH}/src/github.com/spinlock/jemalloc-go/
...
codis-server:
	...
	make -j4 -C extern/redis-6.2.8/
	@cp -f extern/redis-6.2.8/src/redis-server  bin/codis-server
	@cp -f extern/redis-6.2.8/src/redis-benchmark bin/
	@cp -f extern/redis-6.2.8/src/redis-cli bin/
	@cp -f extern/redis-6.2.8/src/redis-sentinel bin/
	@cp -f extern/redis-6.2.8/redis.conf config/
	@sed -e "s/^sentinel/# sentinel/g" extern/redis-6.2.8/sentinel.conf > config/sentinel.conf
...
distclean: clean
	@make --no-print-directory --quiet -C extern/redis-6.2.8 distclean
	@make --no-print-directory --quiet -C  ${GOPATH}/src/github.com/spinlock/jemalloc-go/ distclean
